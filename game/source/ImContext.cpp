...
ImContext::ImContext(f32vec2 const& size, i32vec2 const& windowSize, i32vec2 const& framebufferSize)
		: ImContext{*ImGui::CreateContext(), *ImPlot::CreateContext(), size, windowSize, framebufferSize}
{}
...
ImContext::ImContext(ImGuiContext& context, ImPlotContext& plotCtx, f32vec2 const& size, i32vec2 const& windowSize,
                     i32vec2 const& framebufferSize)
		: _context{&context}, _plotCtx{&plotCtx}, _shader{Shaders::Flat2D::Flag::Textured | Shaders::Flat2D::Flag::VertexColor}
{
	makeCurrent();
...
ImContext::ImContext(ImGuiContext& context, ImPlotContext& plotCtx, i32vec2 const& size)
		: ImContext{context, plotCtx, f32vec2{size}, size, size}
{}

ImContext::ImContext(Magnum::NoCreateT) noexcept
		: _context{nullptr}, _plotCtx{nullptr}, _shader{NoCreate}, _texture{NoCreate}, _vertexBuffer{NoCreate}, _indexBuffer{NoCreate},
		  _mesh{NoCreate}
{}

ImContext::ImContext(ImContext&& other) noexcept
		: _context{other._context}, _plotCtx{other._plotCtx}, _shader{std::move(other._shader)}, _texture{std::move(other._texture)},
